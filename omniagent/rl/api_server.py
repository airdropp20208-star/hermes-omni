...
FastAPI proxy server between external agents and SGLang inference engine.
...
def _extract_logprobs_from_chat_response(choice: Dict[str, Any]) -> List[float]:
    """Extract per-token logprob values from SGLang chat completion response."""
...
def _build_prm_judge_prompt(
    ...
    """Build the PRM judge prompt from response + next state."""
    system = (
        ...
        "  * The user requests a **correction or modification** to what the assistant "
        "just did (\"change X to Y\", \"no, I meant …\", \"not that, …\", "
        "\"please fix …\").\n"
        "  * The user **rephrases or restates** the same request, implying the "
        "assistant did not understand or execute it correctly.\n"
        "  * The environment returns an **error, failure, or unexpected result** "
        "caused by the assistant's action.\n"
...
def _majority_vote(scores: List[Optional[int]]) -> float:
    """Majority vote over PRM scores. Ties return 0.0."""
...
# ── SLIME plugin functions ──────────────────────────────────────


async def reward_func(args, sample_or_samples, **kwargs):
    """Custom reward function for SLIME trainer.

    Extracts the 'score' key from sample.reward dict.
    Referenced by: --custom-rm-path omniagent.rl.api_server.reward_func
    """
...
async def generate(args, sample, sampling_params, evaluation: bool = False):
    """Custom generate function for SLIME eval rollout.

    Tokenizes prompt, sends to SGLang /generate endpoint, extracts logprobs.
    Referenced by: --custom-generate-function-path omniagent.rl.api_server.generate
    """
...
class RLAPIServer:
    """Proxy between external agents and SGLang for RL training data collection.

    External agents (e.g. OmniAgent) send ``X-Session-Id`` and ``X-Turn-Type``
    headers with every request.  The proxy forwards to SGLang, and when
    ``turn_type`` is ``"main"`` it tokenizes the full prompt+response and
    ...
    def __init__(
        ...
        self.tokenizer = self._load_tokenizer(args.hf_checkpoint)
        ...
        self.served_model_name = os.getenv(
            "SERVED_MODEL_NAME", getattr(args, "served_model_name", "qwen3-8b"),
        )
        ...
        self._prm_url = (
            f"http://{prm_ip}:{prm_port}/generate"
            if prm_ip and prm_port
            else ""
        )
        self._prm_tokenizer = None
        if self._prm_enabled:
            prm_path = getattr(args, "prm_model_path", None) or args.hf_checkpoint
            self._prm_tokenizer = self._load_tokenizer(prm_path)
            logger.info("[OmniAgent-RL] PRM enabled: url=%s m=%d", self._prm_url, self._prm_m)
        ...
        # Record files
        self._record_file = (
            os.getenv("RL_RECORD_FILE", "")
            if os.getenv("RL_RECORD_ENABLED", "0") == "1"
            else ""
        )
        ...
        self._prm_record_file = os.getenv("RL_PRM_RECORD_FILE", "")
    ...
    def _load_tokenizer(path: str):
        """Load tokenizer with SLIME utility or fall back to transformers."""
        try:
            from slime.utils.processing_utils import load_tokenizer
    ...
    def _build_app(self) -> FastAPI:
        app = FastAPI(title="OmniAgent RL Proxy")
        ...
        async def chat_completions(
            ...
            owner: RLAPIServer = request.app.state.owner
            await owner._check_auth(authorization)
            if not owner.submission_enabled.is_set():
                raise HTTPException(
                    status_code=503,
                    detail="submission paused for weight update",
                )

            body = await request.json()
            session_id = x_session_id or body.get("session_id") or "unknown"
            turn_type = (
                (x_turn_type or body.get("turn_type") or "side").strip().lower()
            )
            session_done = (
    ...
    def _flush_pending_record(self, session_id: str, next_state):
        ...
        if next_state:
            ...
            logger.info(
                f"{_GREEN}[OmniAgent-RL] session={session_id} turn={rec['turn']} "
                f"next_state role={ns_role} len={len(ns_content)}: "
                f"{ns_content[:200]}{_RESET}",
            )
            self._fire_prm_scoring(
                session_id, rec["turn"], rec["response_text"], next_state,
            )
        if self._record_file:
            try:
                ...
                logger.warning("[OmniAgent-RL] failed to write record: %s", e)
    ...
    def _append_prm_record(
        ...
        try:
            ...
            logger.warning("[OmniAgent-RL] failed to write PRM record: %s", e)
    ...
    def purge_record_files(self):
        ...
        for path, label in [
            ...
            try:
                ...
                logger.warning(
                    "[OmniAgent-RL] failed to purge %s file: %s", label, e,
                )

    # ── PRM scoring ────────────────────────────────────────────

    async def _query_prm_once(
        self, judge_prompt: str, vote_id: int,
    ) -> Tuple[Optional[int], str]:
        """Single PRM query to the PRM SGLang endpoint."""
        ...
        try:
            ...
            logger.warning(
                "[OmniAgent-RL] PRM query failed (vote %d): %s", vote_id, e,
            )
            return None, ""
    ...
    async def _prm_evaluate(
        ...
        """Run m parallel PRM queries and apply majority vote."""
        ns_text = (
        ...
        logger.info(
            f"{_CYAN}[OmniAgent-RL] PRM session={session_id} turn={turn_num} "
            f"votes={votes_display} → score={final}{_RESET}",
        )
        self._append_prm_record(
            session_id, turn_num, final, votes_display, representative,
        )
        return {
            "score": final,
            "votes": votes_display,
    ...
    def _fire_prm_scoring(
        ...
        """Fire async PRM evaluation task."""
    ...
    async def _handle_request(
        ...
        """Core request flow: forward to SGLang, tokenize, collect sample."""
        ...
        # Strip non-standard keys, force stream=False, enable logprobs
        forward_body = {
            k: v for k, v in body.items() if k not in _NON_STANDARD_BODY_KEYS
        }
        ...
        async with httpx.AsyncClient(timeout=None) as client:
            ...
            if sglang_resp.status_code != 200:
                logger.error(
                    "[OmniAgent-RL] SGLang returned %d: %s",
            ...
            try:
                ...
                logger.error(
                    "[OmniAgent-RL] SGLang non-JSON body: %s",
                    sglang_resp.text[:1000],
                )
                raise
        ...
        logger.info(
            f"{_YELLOW}[OmniAgent-RL] [{turn_type}] session={session_id} "
            f"prompt_msgs={len(messages)}{_RESET}",
        )
        logger.info(
            f"{_RED}[OmniAgent-RL] [{turn_type}] session={session_id} "
            f"thinking={len(reasoning)} chars, response:\n{content}{_RESET}",
        )
        if tool_calls:
            logger.info(
                "[OmniAgent-RL] tool_calls: %s",
                json.dumps(tool_calls, ensure_ascii=False)[:500],
            )

        if turn_type == "main":
            # Flush previous turn's record (its next_state = current last message)
            ...
            if full_text.startswith(prompt_text):
                ...
                logger.warning(
                    "[OmniAgent-RL] prompt_text is not a prefix of full_text",
                )
            ...
            if not response_ids and not response_text.strip():
                logger.info(
                    "[OmniAgent-RL] MAIN session=%s → empty response, skipping",
                    session_id,
                )
                output["session_id"] = session_id
                return {"response": output}

            # Extract and align logprobs
        ...
        if session_done:
            ...
            logger.info(
                "[OmniAgent-RL] session=%s done → cleaned up (effective_samples=%d)",
                session_id, eff,
            )
    ...
    async def _submit_turn_sample(
        ...
        """Construct a Sample object and push to the output queue."""
        from slime.utils.types import Sample
        ...
        # At-least-one guarantee: promote score=0 if session has zero effective samples
        if (
            exclude
            and has_next_state
            ...
            exclude = False
            logger.info(
                "[OmniAgent-RL] promoting session=%s turn with score=0 → "
                "loss_mask=1 (at-least-one guarantee)",
                session_id,
            )
        ...
        sample.loss_mask = (
            [0] * len(response_ids) if exclude else [1] * len(response_ids)
        )
        ...
        logger.info(
            "[OmniAgent-RL] submitted sample session=%s index=%d "
            "score=%.1f exclude=%s prompt_len=%d response_len=%d",
            session_id, sample.index, score, exclude,
            len(prompt_ids), len(response_ids),
        )
        await asyncio.to_thread(
            self.output_queue.put, (sample.group_index, [sample]),
        )
    ...
    def _task_done_cb(task: asyncio.Task):
        ...
        if exc is not None:
            logger.error(
                "[OmniAgent-RL] background task failed: %s", exc, exc_info=exc,
            )

    # ── Streaming ──────────────────────────────────────────────

    async def _stream_response(self, result: Dict[str, Any]):
        """Convert a non-streamed response into SSE chunks."""
    ...
    def _wait_for_sglang_ready(self):
        """Poll SGLang and PRM health endpoints until ready."""
        while True:
            try:
                ...
                pass
            time.sleep(3)
        logger.info("[OmniAgent-RL] policy server ready")
        ...
        if self._prm_enabled and self._prm_url:
            ...
            while True:
                try:
                    ...
                    pass
                time.sleep(3)
            logger.info("[OmniAgent-RL] PRM server ready")
        ...
        banner = (
            f"\n{'=' * 70}\n"
            f"  [OmniAgent-RL] your model is fired up and ready to roll\n"
