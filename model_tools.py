...
def handle_function_call(
    ...
    try:
        ...
        # Unified guardian is wired directly into the dispatcher so Hermes gets
        # OmniAgent-style sentinel checks even when the optional plugin layer is
        # not loaded. Fail-open: the integration module is responsible for
        # swallowing its own errors.
        try:
            from agent.unified.integration import before_tool_call, after_tool_call

            unified_block_message = before_tool_call(
                tool_name=function_name,
                args=function_args,
                task_id=task_id or "",
                session_id=session_id or "",
            ...
            logger.debug("unified pre-tool integration error: %s", _unified_err)
            unified_block_message = None
            after_tool_call = None  # type: ignore[assignment]

        if unified_block_message is not None:
            result = json.dumps({"error": unified_block_message}, ensure_ascii=False)
            try:
                after_tool_call(
                    tool_name=function_name,
                    args=function_args,
            ...
            _emit_post_tool_call_hook(
                ...
                status="blocked",
                error_type="unified_guardian",
                error_message=unified_block_message,
        ...
        # Unified reflexion is core-integrated: failed/blocked tool results are
        # converted into durable lessons available through unified_recall.
        try:
            if after_tool_call is not None:  # type: ignore[name-defined]
                after_tool_call(
                    tool_name=function_name,
                    args=function_args,
                    result=result,
